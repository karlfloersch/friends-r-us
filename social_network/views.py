...
def search_for_a_user_add_to_Circle(circle_id, real_first_name, real_last_name):
    ...
    cursor.execute("SELECT U.User_Id, U.First_Name, U.Last_Name FROM User U where U.First_Name LIKE %"+real_first_name+"% OR U.Last_Name LIKE %"+real_last_name+"%")
    curst_id = cursor.fetchone()
    cursor.execute("INSERT INTO AddedTo VALUES ("+cust_id+","+circle_id+")")


def create_a_circle(owner_id,name, circle_type):
    cursor = connection.cursor()
    cursor.execute("INSERT INTO Circle VALUES( NULL,"+owner_id+","+name+","+circle_type+")")
...
def make_a_post(customer_id, page_id):
    ...
    cursor = connection.cursor()
    cursor.execute("insert into post values(NULL,"+date_now+","+ts+",0,"+customer_id+","+page_id+")")
...
def comment_on_a_post(content,author_id, post_id):
    ...
    ts = date.isoformat()
    cursor = connection.cursor()
    cursor.execute("INSERT INTO comment VALUES (NULL,"+date_now+","+ts+","+content+","+author_id+","+post_id+")")
    cursor.execute("UPDATE post SET Comment_Count = Comment_Count + 1 WHERE Post_Id ="post_id)
...
def like_comment(comment_id,cust_id):
    ...
    # cust_id    INT,
    cursor = connection.cursor()
    cursor.execute("INSERT INTO likecomment VALUES ("+comment_id+","+cust_id+")")
...
def remove_a_comment(comment_id):
    ...
    cursor = connection.cursor()
    cursor.execute("UPDATE post SET comment_Count = comment_Count -1 WHERE post_id IN (SELECT C.Post FROM comment C WHERE C.comment_id = "+comment_id+")")
...
# post_id INT, 
    ...
    cursor = connection.cursor()
    cursor.execute("DELETE FROM likepost WHERE post_id = "+post_id+" AND cust_id ="+cust_id)


def unlike_a_comment(comment_id, cust_id):
    cursor = connection.cursor()
    cursor.execute("DELETE FROM likecomment WHERE comment_id = "+comment_id+" AND cust_id="+cust_id);
...
def modify_a_comment(content, comment_id):
    ...
    cursor = connection.cursor()
    cursor.execute("UPDATE Comment SET Content = "+content+" WHERE id ="+comment_id)
...
def join_a_circle(cust_id,circle_id):
    ...
    cursor = connection.cursor()
    cursor.execute("Insert into memberofcircle values ("+cust_id+","+circle_id+")")
def unjoin_a_circle(cust_id,circle_id):
    # DELETE FROM AddedTo WHERE user=? AND circle = ?  
    cursor = connection.cursor()
    cursor.execute("DELETE FROM memberofcircle where cust_id ="+cust_id+"AND circle_id="+circle_id)
...
def send_recieve_message(content,sender_id,receiver_id,subject):
    ...
    cursor = connection.cursor()
    date_now = time.strftime("%d/%m/%Y")
    cursor.execute("INSERT INTO message(NULL,"+content+","+sender_id+","+receiver_id+","+date_now+")")
...
def this_users_circles(user_id):
    cursor = connection.cursor()
    value_to_exec= "SELECT C.* FROM Circle WHERE C.owner_id = "+owner_id+" UNION SELECT C.* FROM Circle C INNER JOIN memberofcircle A ON C.id = A.circle_id WHERE A.User_Id = "+user_id
